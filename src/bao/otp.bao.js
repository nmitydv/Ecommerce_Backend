...
const { OtpDao, UserDao, StoreAssociateDao } = require("../dao");
...
class OtpBao extends Base {
  ...
  async verifyOtp(emailId, otp, status, type) {
    try {
      ...
      if (emailId.toLowerCase() === "demo@vetir.com" && otp === 9999) {
        ...
        if (otpDetails.length > 0) {
          ...
          if (type && type === 'stylist') {
            userDetails = await UserDao.findStylistByEmail(emailId.toLowerCase());
            return {
              statusCode: constants.STATUS_CODES[200],
              statusMessage: "otp verified successfully",
              personalStylistId: userDetails[0]._id,
              emailId: userDetails[0].emailId,
              isProfileCreated: userDetails[0].isProfileCreated ? userDetails[0].isProfileCreated : false,
            };
          } else if (type && type === 'storeassociate') {
            userDetails = await StoreAssociateDao.findStoreAssociateByEmail(emailId.toLowerCase());
            return {
              statusCode: constants.STATUS_CODES[200],
              statusMessage: "otp verified successfully for storeassociate",
              storeAssociateId: userDetails[0]._id,
              emailId: userDetails[0].emailId,
              isProfileCreated: userDetails[0].isProfileCreated ? userDetails[0].isProfileCreated : false,
            };
          } else {
            userDetails = await UserDao.findUserEmailId(emailId);
            return {
              statusCode: constants.STATUS_CODES[200],
              statusMessage: "otp verified successfully",
              userId: userDetails[0].userId,
