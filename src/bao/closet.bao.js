...
const { ClosetDao, UserDao, OutfitDao, AdminDao } = require("../dao");
...
class ClosetBao extends Base {
  ...
  async getBrands() {
    try {
      ...
      if (cachedData) {
        ...
        // let result = await AdminDao.getAllBrands()
        let result = await ClosetDao.getBrands()
        // result = result.map((brand) => {
        //   return {
        //     _id: brand._id,
        //     brandId: brand._id,
        //     brandName: brand.name ? brand.name : 'NA'
        //   }
        // })
        cache.put(cacheKey, result, 24 * 60 * 60 * 1000);
        return {
  ...
  async getAdminBrands() {
    try {
      // logger.info("inside getAdminBrands");
      let cacheKey = "adminBrands";
      let cachedData = cache.get(cacheKey);
      if (cachedData) {
        return {
          statusCode: constants.STATUS_CODES[200],
  ...
  async getSizes() {
    try {
      ...
      if (cachedData) {
        ...
        result = result.sort((a, b) => a.sizeId - b.sizeId)
        cache.put(cacheKey, result, 24 * 60 * 60 * 1000);
        return {
          statusCode: constants.STATUS_CODES[200],
          statusMessage: constants.STATUS_MESSAGE[200],
  ...
  async addToCloset(closetData) {
    try {
      console.log(closetData, "inside addToCloset")
      let userDetails = await UserDao.findUserId(closetData.userId);
      if (userDetails.length == 0) {
        let stylistData = await UserDao.findStylistById(closetData.userId)
      ...
      if (closetData.isImageBase64 || closetData.itemImageUrl.length > 200) {
        let base64Data = closetData.itemImageUrl.match(
          /^data:([A-Za-z-+\/]+);base64,(.+)$/
        );
        if (base64Data && base64Data.length !== 3 || base64Data == null) {
          return {
            statusCode: constants.STATUS_CODES[304],
            statusMessage: constants.STATUS_MESSAGE[304],
          };
        }
        uploadFileResult = await S3Service.uploadClosetPicToS3(
          base64Data
        );
      } else {
        uploadFileResult = {
          Location: closetData.itemImageUrl
        }
      }
      let categoryName;
      let subCategoryName;
      ...
      let findBrandId = (obj) => {
        return obj.brandId ? obj.brandId == closetData.brandId : obj._id.equals(closetData.brandId)
      };
      ...
      brandList.map((element) => {
        if (element.brandId && element.brandId == closetData.brandId) {
          brandName = element.brandName;
        } else if (!element.brandId && element._id.equals(closetData.brandId)) {
      ...
      const insertObj = {
        ...
        brandName: brandName ? brandName : 'NA',
        season: closetData.season && closetData.season.length > 0 ? closetData.season.map(function (x) {
          return x.toLowerCase();
        }) : [],
        colorCode: closetData.colorCode.map(function (x) {
          return x.toLowerCase();
        }),
        price: closetData.price,
        notes: closetData.notes,
        createdOn: new Date().toISOString(),
        updatedOn: new Date().toISOString(),
      };
      if (closetData.productId) {
        insertObj.productId = closetData.productId
      }
  ...
  async getClosetDetails(userId, sort) {
    try {
      // logger.info("inside getClosetDetails");
      let sortBy = sort ? sort : 'asc'
      ...
      const closetDetails = await ClosetDao.getClosetDetails(userId, sortBy, 100, 1);
      let data = [];
      ...
      return {
        statusCode: constants.STATUS_CODES[200],
        statusMessage: constants.STATUS_MESSAGE[200],
        data: data.length > 0 ? data.sort((a, b) => new Date(a.createdOn) - new Date(b.createdOn)) : []
      };
    } catch (e) {
      console.log(e)
      // logger.error(e);
  ...
  async getClosetDetailsV1(params) {
    try {
      const closetDetails = await ClosetDao.getClosetAll()
      closetDetails.map(async (x) => {
        let base64Data = x.itemImageUrl.match(
          /^data:([A-Za-z-+\/]+);base64,(.+)$/
        );
        if (base64Data && base64Data.length !== 3 || base64Data == null) {
          return {
        ...
        let uploadFileResult = await S3Service.uploadClosetPicToS3(
          base64Data
        );
  ...
  async getOneClosetDetails(userId, closetId) {
    try {
      console.log(userId, closetId, "GETONECLOST")
  ...
  async filterCloset(params) {
    try {
      ...
      if (userDetails.length == 0) {
        let stylistData = await UserDao.findStylistById(params.userId)
        if (stylistData.length == 0) {
          return {
            statusCode: constants.STATUS_CODES[302],
            statusMessage: constants.STATUS_MESSAGE[302],
  ...
  async editClosetDetails(closetData) {
    try {
      ...
      if (closetDetails.length > 0) {
        ...
        if (typeof closetData.price !== 'undefined') {
          updateObj = {
            price: closetData.price,
            updatedOn: new Date().toISOString(),
          };
        ...
        if (typeof closetData.notes !== 'undefined') {
          updateObj = {
            notes: closetData.notes,
            updatedOn: new Date().toISOString(),
          };
        ...
        return {
          ...
          price: finalClosetDetails[0].price,
          notes: finalClosetDetails[0].notes,
        };
      } else {
        return {
          statusCode: constants.STATUS_CODES[308],
