...
const { UserDao, OtpDao, ClosetDao, AdminDao, StoreAssociateDao } = require("../dao");
...
class AuthBao extends Base {
  ...
  async login(emailId, status, type) {
    try {
      // logger.info("inside login", emailId);
      const otpRes = await this.sendOtp(emailId, status);
      let userId = -1;
      let isProfileCreated = false;
      let findEmailId, checkStylistClients
      if (type && type === 'stylist') {
        findEmailId = await UserDao.findStylistEmailId(emailId.toLowerCase());
      } else if (type && type == 'storeassociate'){
        findEmailId = await StoreAssociateDao.findEmailId({ emailId })
      } else {
        findEmailId = await UserDao.findUserEmailId(emailId);
      }
      if (findEmailId.length > 0) {
        ...
        if (otpRes) {
          if (type && type === 'stylist') {
            checkStylistClients = await UserDao.findStylistClient(findEmailId[0]._id);
            console.log(findEmailId, checkStylistClients)
            if (checkStylistClients.length > 0) {
              return {
                statusCode: constants.STATUS_CODES[200],
                statusMessage: "OTP sent successfully",
                personalStylistId: findEmailId[0]._id,
              ...
              return {
                ...
                statusMessage: 'Your Client has not added you as a stylist',
              };
            }
          } else {
            return {
        ...
        if (type && type === 'stylist') {
          ...
          return {
            statusCode: constants.STATUS_CODES[303],
            statusMessage: 'Your Store has not added you as an associate',
          };
        }
        do {
